...
function renderActivityData(thisData){
    let str ="";
    thisData.forEach(item=>{
        ...
        <nav aria-label="breadcrumb ">
            <ol class="breadcrumb mb-4 mb-lg-5">
                ...
                <li class="breadcrumb-item active" aria-current="page">${item.ActivityName}</li> 
            </ol>
        </nav>
        <div class=" overflow-hidden radius-20 mb-4  mb-lg-5">
            <img src="${item.Picture.PictureUrl1}" class="h-185  h-lg-400 w-100" alt="${item.Picture.PictureDescription1}">
        </div>
            <h2 class="fs-3 fs-lg-2 fw-lighter mb-3 letter-spacing">${item.ActivityName}</h2>
            <p class="mb-4 mb-lg-5">
                <span class="category-tag fs-5 radius-20  mb-5"># ${item.Class1}</span>
                <span class="category-tag fs-5 radius-20  mb-5"># ${item.Class2}</span>
            </p>
        <div class="mb-5">
            <h4 class="fs-5 fw-bold mb-3">餐廳介紹：</h4>
            <p class="fw-lighter lineHeight-lg">${item.Description}</p>
        </div>
        <div class="row">
            <div class="col-lg-6 ">
                <div class="infrom-descrp  bg-foodDescrption  h-100">
                    <p class="fw-bold">活動時間： <span class="fw-normal"> ${activityStartTime} - ${activityEndTime} </span> </p>
                    <p class="fw-bold">聯絡電話： <span class="fw-normal"> ${item.Phone}</span> </p>
                    <p class="fw-bold">主辦單位： <span class="fw-normal"> ${item.Organizer}</span> </p>
                    <p class="fw-bold">活動地點： <span class="fw-normal"> ${item.Location}</span> </p>
                    <p class="fw-bold">官方網站： <a href="${item.WebsiteUrl}" trage="_blank" class="fw-normal"> ${item.WebsiteUrl  != undefined  ? "  item.WebsiteUrl" : "未提供網址"  } </a></p>
                    <p class="fw-bold">活動費用： <a href="#" class="fw-normal"> ${item.Charge}</a> </p>
                    <p class="fw-bold">注意事項： <span class="fw-normal"> ${item.Remarks}</span></p>
                </div>
            </div>
            <div class="col-lg-6">
                <div class="">
        ...
        dd.forEach(item=>{
            ...
            <a href="activity_page.html?id=${item.ActivityID}">
                <div class="card border-0 mb-3 h-100" >
                    <div class="overflow-hidden radius-20 category">
                        <img src="${item.Picture.PictureUrl1}" style="height:200px" class="w-100 card-img-top card-img img-transition h-200" alt="${item.Picture.PictureDescription1}">
                    </div>
                    <div class="card-body p-0">
                        <h3 class="card-title fw-bold text-dark text-truncate">${item.ActivityName}</h3>
