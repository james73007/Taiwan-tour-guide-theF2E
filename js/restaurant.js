...
const navbar = document.querySelector('.navbargg');
const d = document.querySelector('.d');
const jsGoTop = document.querySelector('.js-goTop');
...
function renderData(currentData) {
    ...
    let str="";
    str+=`<h2 class="fw-lighter fs-2 letter-spacing">搜尋結果 <sphn class="fw-light fs-4">共有${foodData.length} 筆</span></h2>`;
    currentData.forEach(item=>{
        str+=`
            <div class="col-md-6 col-lg-3 mb-3 ">
                <div class="card border-0 mb-3 h-100 w-100" >
                    <a href="restaurant_page.html?id=${item.RestaurantID}" class="d-block">
                    <div class="card-body p-0  category">
                        <div class="h-200  w-100 overflow-hidden" >
                            <img src="${item.Picture.PictureUrl1}" class="h-100 w-100 radius-20 img-transition" alt="${item.Picture.PictureDescription1}">
                        </div>
                        <h4 class="card-title text-dark text-truncate">${item.RestaurantName}</h4> 
                        <p class="card-text text-light"><i class="fas fa-map-marker-alt"></i>${item.City}</p>
                    </div>
                    </a>
                </div>
            </div>`
...
function pagination(nowPage){
        ...
        const pageTotal = Math.ceil(foodData.length/perpage);  //無條件進位   
